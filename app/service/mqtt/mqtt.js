...
// const MQTT_URL = 'ws://staging-dev-mqtt.safetravel.ph:8000/mqtt';
...
export const onMqttPuvVehicleSubscribe = (topic, onMessageCallback) => {
...
export const onMqttCommuterSubscribe = (topic, onMessageCallback) => {
    console.log('SUBSCRIBED', topic);
    ...
    try {
        ...
        client.on('message', (topic, message) => {
            if (topic === 'commuters') {
                ...
                console.warn('mqtt commuter subscribe', data);
                const coordinates = {latitude: Number(data.lat), longitude: Number(data.lng), userId: data.userId}
